import { X, CheckCircle2, Zap, Shield, Package, ArrowUpRight, Calculator, FileText } from "lucide-react";
...
const ProductModal = ({ isOpen, onClose, productId, productType }: ProductModalProps) => {
    ...
    const dominicanCompliance = getData(`${basePath}.dominicanCompliance`);
    const hrModules = getData(`${basePath}.hrModules`);
    const payrollFeatures = getData(`${basePath}.payrollFeatures`);

    // Specific Data for Salary Calc
    const howItWorks = getData(`${basePath}.howItWorks`);
    const isrScale = getData(`${basePath}.isrScale`);
                        ...
                        >
                            ...
                            <div className="overflow-y-auto custom-scrollbar p-6 md:p-8 bg-theme">
                                ...
                                >
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                                {features.map((feature: any, idx: number) => (
                                                    <div key={idx} className="bg-surface border border-theme rounded-xl p-4 hover:bg-surface-2 transition-colors flex items-start gap-3 group">
                                                        ...
                                                        <span className="text-muted font-medium">{feature.label || feature}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </motion.div>
                                        ...
                                        <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                            <div className="bg-surface rounded-3xl p-6 md:p-8 border border-theme">
                                                ...
                                                <ul className="space-y-4">
                                                    {dominicanCompliance.items?.map((item: string, idx: number) => (
                                            ...
                                            {payrollFeatures?.items && (
                                                <div className="bg-gradient-to-br from-purple-900/10 to-blue-900/10 rounded-3xl p-6 md:p-8 border border-theme">
                                                    ...
                                                    <ul className="space-y-4">
                                                        {payrollFeatures.items?.map((item: string, idx: number) => (
                                                            <li key={idx} className="flex items-start gap-3">
                                    ...
                                    {howItWorks?.deductions && (
                                        <motion.div variants={itemVariants} className="space-y-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <h3 className="text-2xl font-bold text-theme">{howItWorks.title}</h3>
                                            </div>
                                            <p className="text-muted">{howItWorks.intro}</p>
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                {howItWorks.deductions?.map((deduction: any, idx: number) => (
                                                    <div key={idx} className="bg-surface p-4 rounded-xl border border-theme">
                                                        <h4 className="font-bold text-theme mb-1">{deduction.name}</h4>
                                                        <div className="text-xs font-mono text-purple-500 mb-2">{deduction.rate}</div>
                                                        <p className="text-sm text-muted">{deduction.desc}</p>
                                                    </div>
                                                ))}
                                            </div>
                                            {howItWorks.note && (
                                    ...
                                    {/* Tools: ISR Scale */}
                                    {isrScale?.brackets && (
                                        <motion.div variants={itemVariants} className="bg-surface rounded-2xl border border-theme overflow-hidden">
                                            ...
                                            <div className="divide-y divide-border">
                                                {isrScale.brackets?.map((bracket: any, idx: number) => (
                                                    <div key={idx} className="p-4 flex flex-col sm:flex-row sm:justify-between gap-2 hover:bg-surface-2 transition-colors">
                                                        <span className="font-mono text-sm text-muted">{bracket.range}</span>
                                                        <span className="font-medium text-theme">{bracket.rate}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </motion.div>
                                    ...
                                    {/* CTA Footer */}
                                    {link && (
                                        <motion.div variants={itemVariants} className="flex justify-end pt-6 border-t border-theme">
                                            <a
                                                href={link}
                                                target="_blank"
